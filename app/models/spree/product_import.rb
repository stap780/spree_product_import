module Spree
    class ProductImport < Spree::Base
        ...
        def self.import_attributes
            ...
            variants = Spree::Variant.attribute_names.map{|fi| [fi,'variant#'+fi] if !not_use_variant_attr.include?(fi)}

            products = Spree::Product.attribute_names.map{|fi| [fi,'product#'+fi] if !not_use_product_attr.include?(fi)}+[['price','product#price'],['cat1','product#cat1'],['cat2','product#cat2'],['cat3','product#cat3'],['quantity','product#quantity'],['images','product#images']]
            our_fields['product'] = products.reject(&:blank?)+variants.reject(&:blank?)
